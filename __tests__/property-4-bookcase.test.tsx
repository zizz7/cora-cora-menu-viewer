...
const pcArb = fc.integer({ min: 1, max: 100 });

const menusArb: fc.Arbitrary<MenuMeta[]> = fc.oneof(
  pcArb.map((pc) => [makeMenu(0, pc)]),
  fc.tuple(pcArb, pcArb).map(([a, b]) => [makeMenu(0, a), makeMenu(1, b)]),
  fc.tuple(pcArb, pcArb, pcArb).map(([a, b, c]) => [
    makeMenu(0, a), makeMenu(1, b), makeMenu(2, c),
  ]),
  fc.tuple(pcArb, pcArb, pcArb, pcArb).map(([a, b, c, d]) => [
    makeMenu(0, a), makeMenu(1, b), makeMenu(2, c), makeMenu(3, d),
  ]),
);
